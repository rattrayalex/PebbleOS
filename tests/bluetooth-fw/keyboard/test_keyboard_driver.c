...
// Advertisements are length-type-value triples. Building real ones keeps the driver's parsing,
// name clamping and service-identifier search on the tested path.
#define AD_TYPE_FLAGS      0x01
#define AD_TYPE_UUID16_ALL 0x03
#define AD_TYPE_NAME_FULL  0x09
#define AD_TYPE_APPEARANCE 0x19

// Legacy advertising carries at most 31 bytes; the buffer is larger so a test can hand the driver
// a longer payload than any legacy advertiser could send.
typedef struct {
  uint8_t data[64];
  uint8_t length;
} AdvData;

static void prv_ad_field(AdvData *ad, uint8_t type, const void *value, uint8_t length) {
  cl_assert(ad->length + 2u + length <= sizeof(ad->data));
  ad->data[ad->length++] = length + 1;
  ad->data[ad->length++] = type;
  memcpy(&ad->data[ad->length], value, length);
  ad->length += length;
}

static void prv_ad_flags(AdvData *ad) {
  const uint8_t flags = 0x06; // General discoverable, BR/EDR not supported.
  prv_ad_field(ad, AD_TYPE_FLAGS, &flags, sizeof(flags));
}

static void prv_ad_appearance(AdvData *ad, uint16_t appearance) {
  const uint8_t value[] = {appearance & 0xff, appearance >> 8};
  prv_ad_field(ad, AD_TYPE_APPEARANCE, value, sizeof(value));
}

static void prv_ad_uuids16(AdvData *ad, const uint16_t *uuids, size_t count) {
  uint8_t value[12];
  cl_assert(count * 2 <= sizeof(value));
  for (size_t i = 0; i < count; ++i) {
    value[i * 2] = uuids[i] & 0xff;
    value[i * 2 + 1] = uuids[i] >> 8;
  }
  prv_ad_field(ad, AD_TYPE_UUID16_ALL, value, count * 2);
}

static AdvData prv_keyboard_ad(void) {
  AdvData ad = {0};
  prv_ad_flags(&ad);
  prv_ad_appearance(&ad, 0x03c1);
  return ad;
}

static AdvData prv_other_ad(void) {
  AdvData ad = {0};
  prv_ad_flags(&ad);
  prv_ad_appearance(&ad, 0x0040); // Generic phone.
  return ad;
}

static void prv_report_adv(uint8_t event_type, const ble_addr_t *addr, const AdvData *ad) {
  prv_gap((struct ble_gap_event){
    .type = BLE_GAP_EVENT_DISC,
    .disc = {.event_type = event_type, .addr = *addr, .data = ad->data, .length_data = ad->length}
  });
}

static ble_addr_t prv_addr(uint8_t seed) {
  ble_addr_t addr = {.type = BLE_ADDR_RANDOM, .val = {seed, 0x22, 0x33, 0x44, 0x55, 0xc6}};
  return addr;
}
...
static void prv_start_pair(void) {
  ...
  const AdvData ad = prv_keyboard_ad();
  prv_report_adv(BLE_HCI_ADV_RPT_EVTYPE_ADV_IND, &s_saved, &ad);
  prv_connected();
}
...
static void prv_start_scan(void) {
  nimble_keyboard_start();
  prv_drain();
  bt_keyboard_pair();
  prv_drain();
...
void test_keyboard_driver__scan_response_requires_prior_connectable_advertisement(void) {
  prv_start_scan();
  const AdvData keyboard = prv_keyboard_ad();
  const AdvData other = prv_other_ad();
  // A scan response alone proves nothing about connectability.
  prv_report_adv(BLE_HCI_ADV_RPT_EVTYPE_SCAN_RSP, &s_saved, &keyboard);
  cl_assert_equal_i(0, s_connect_count);
  // A connectable advertisement that does not identify as a keyboard is only remembered.
  prv_report_adv(BLE_HCI_ADV_RPT_EVTYPE_ADV_IND, &s_saved, &other);
  cl_assert_equal_i(0, s_connect_count);
  prv_report_adv(BLE_HCI_ADV_RPT_EVTYPE_SCAN_RSP, &s_saved, &keyboard);
  cl_assert_equal_i(1, s_connect_count);
}

// Defect: the driver used to remember only the four most recent connectable advertisers, without
// discarding repeats, so one chatty neighbour evicted the keyboard before its scan response
// arrived and pairing failed with "Could not connect" for as long as the room stayed the same.
void test_keyboard_driver__chatty_neighbour_does_not_evict_the_keyboard(void) {
  prv_start_scan();
  const AdvData keyboard = prv_keyboard_ad();
  const AdvData other = prv_other_ad();
  // The keyboard's advertisement has no room for the HID marker, which rides its scan response.
  prv_report_adv(BLE_HCI_ADV_RPT_EVTYPE_ADV_IND, &s_saved, &other);
  cl_assert_equal_i(0, s_connect_count);
  const ble_addr_t neighbour = prv_addr(0x40);
  for (unsigned i = 0; i < 200; ++i) {
    prv_report_adv(BLE_HCI_ADV_RPT_EVTYPE_ADV_IND, &neighbour, &other);
  }
  cl_assert_equal_i(0, s_connect_count);
  prv_report_adv(BLE_HCI_ADV_RPT_EVTYPE_SCAN_RSP, &s_saved, &keyboard);
  cl_assert_equal_i(1, s_connect_count);
...
void test_keyboard_driver__a_roomful_of_distinct_advertisers_keeps_the_keyboard(void) {
  prv_start_scan();
  const AdvData keyboard = prv_keyboard_ad();
  const AdvData other = prv_other_ad();
  prv_report_adv(BLE_HCI_ADV_RPT_EVTYPE_ADV_IND, &s_saved, &other);
  for (unsigned i = 0; i < 15; ++i) {
    const ble_addr_t addr = prv_addr(i);
    prv_report_adv(BLE_HCI_ADV_RPT_EVTYPE_ADV_IND, &addr, &other);
  }
  prv_report_adv(BLE_HCI_ADV_RPT_EVTYPE_SCAN_RSP, &s_saved, &keyboard);
  cl_assert_equal_i(1, s_connect_count);
}

void test_keyboard_driver__hid_service_identifier_among_others_is_found(void) {
  prv_start_scan();
  AdvData ad = {0};
  prv_ad_flags(&ad);
  const uint16_t uuids[] = {0x180f, 0x180a, 0x1812};
  prv_ad_uuids16(&ad, uuids, 3);
  prv_report_adv(BLE_HCI_ADV_RPT_EVTYPE_ADV_IND, &s_saved, &ad);
  cl_assert_equal_i(1, s_connect_count);
}

void test_keyboard_driver__advertisement_without_a_keyboard_marker_is_ignored(void) {
  prv_start_scan();
  AdvData ad = {0};
  prv_ad_flags(&ad);
  const uint16_t uuids[] = {0x180f, 0x180a};
  prv_ad_uuids16(&ad, uuids, 2);
  prv_ad_appearance(&ad, 0x00c0); // Generic HID, not a keyboard.
  prv_report_adv(BLE_HCI_ADV_RPT_EVTYPE_ADV_IND, &s_saved, &ad);
  cl_assert_equal_i(0, s_connect_count);
}

void test_keyboard_driver__a_malformed_advertisement_is_ignored(void) {
  prv_start_scan();
  AdvData ad = prv_keyboard_ad();
  // A field that claims more bytes than the advertisement carries.
  ad.data[ad.length++] = 0x0a;
  ad.data[ad.length++] = AD_TYPE_NAME_FULL;
  ad.data[ad.length++] = 'k';
  prv_report_adv(BLE_HCI_ADV_RPT_EVTYPE_ADV_IND, &s_saved, &ad);
  cl_assert_equal_i(0, s_connect_count);
}

void test_keyboard_driver__an_advertised_name_is_made_printable(void) {
  prv_start_scan();
  AdvData ad = {0};
  // The longest name a 31-byte advertisement can carry beside the keyboard appearance.
  uint8_t name[25];
  memset(name, 'k', sizeof(name));
  name[0] = 0x01; // Unprintable.
  name[1] = 0x7f; // Unprintable.
  prv_ad_field(&ad, AD_TYPE_NAME_FULL, name, sizeof(name));
  prv_ad_appearance(&ad, 0x03c1);
  cl_assert_equal_i(31, ad.length);
  prv_report_adv(BLE_HCI_ADV_RPT_EVTYPE_ADV_IND, &s_saved, &ad);
  cl_assert_equal_i(1, s_connect_count);
  const BTKeyboardStatus status = prv_status();
  cl_assert_equal_i(sizeof(name), strlen(status.name));
  cl_assert_equal_i('?', status.name[0]);
  cl_assert_equal_i('?', status.name[1]);
  cl_assert_equal_i('k', status.name[2]);
}

// NimBLE rejects a field longer than BLE_HS_ADV_MAX_FIELD_SZ before the driver sees it, so the
// driver's own 31-byte clamp is defence in depth rather than the first line.
void test_keyboard_driver__an_oversized_name_field_is_rejected(void) {
  prv_start_scan();
  AdvData ad = {0};
  uint8_t name[30];
  memset(name, 'k', sizeof(name));
...
void test_keyboard_driver__a_nameless_keyboard_gets_a_fallback_name(void) {
  prv_start_scan();
  const AdvData ad = prv_keyboard_ad();
  prv_report_adv(BLE_HCI_ADV_RPT_EVTYPE_ADV_IND, &s_saved, &ad);
  cl_assert_equal_i(1, s_connect_count);
  cl_assert_equal_s("BLE keyboard", prv_status().name);
}
...
// A name read from an advertisement belongs to whatever device answered the scan, so no outcome
// may be reported against it once the attempt fails. Every publish that ends an unbonded attempt
// is covered here.
void test_keyboard_driver__a_failed_pairing_drops_the_advertised_name(void) {
  prv_start_scan();
  AdvData ad = {0};
  const uint8_t name[] = "Neighbour keyboard";
  prv_ad_field(&ad, AD_TYPE_NAME_FULL, name, sizeof(name) - 1);
  prv_ad_appearance(&ad, 0x03c1);
  prv_report_adv(BLE_HCI_ADV_RPT_EVTYPE_ADV_IND, &s_saved, &ad);
  cl_assert_equal_s("Neighbour keyboard", prv_status().name);
  prv_connected();
  // Authentication refused: terminal, and the name was never ours.
  prv_gap((struct ble_gap_event){
    .type = BLE_GAP_EVENT_PASSKEY_ACTION,
    .passkey = {.conn_handle = 7, .params = {.action = BLE_SM_IOACT_NUMCMP}}
  });
  cl_assert_equal_i(BTKeyboardStateError, prv_status().state);
  cl_assert_equal_i(0, prv_status().name[0]);
}

void test_keyboard_driver__an_interrupted_connection_drops_the_advertised_name(void) {
  prv_start_scan();
  AdvData ad = {0};
  const uint8_t name[] = "Neighbour keyboard";
  prv_ad_field(&ad, AD_TYPE_NAME_FULL, name, sizeof(name) - 1);
...
void test_keyboard_driver__an_explicit_disconnect_drops_the_advertised_name(void) {
  prv_start_scan();
  AdvData ad = {0};
  const uint8_t name[] = "Neighbour keyboard";
  prv_ad_field(&ad, AD_TYPE_NAME_FULL, name, sizeof(name) - 1);
...
// A bonded keyboard's name comes from the store, so a failure must not erase it.
void test_keyboard_driver__a_bonded_name_survives_a_failure(void) {
  s_bonded = true;
  nimble_keyboard_start();
  prv_drain();
  bt_keyboard_connect();
  prv_drain();
  prv_gap((struct ble_gap_event){.type = BLE_GAP_EVENT_CONNECT, .connect.status = BLE_HS_ETIMEOUT});
  cl_assert_equal_s("Keyboard", prv_status().name);
}
